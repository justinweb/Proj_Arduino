...
/* 
 * Function ScI2cMxDisplay6x8Str
 * Desc     ZT.SC-I2CMx Display 6x8 English String
 * Input    addr:ZT.SC-I2CMx Address
            page: location page
            column: location column
            *str: 6x8 English String
 * Output   0 .. success
 *          1 .. length to long for buffer
 *          2 .. address send, NACK received
 *          3 .. data send, NACK received
 *          4 .. other twi error (lost bus arbitration, bus error, ..)
 */
int ZtLib::ScI2cMxDisplay6x8Str(uint8_t addr, uint8_t page, uint8_t column, const char *str)
{
    uint8_t i=0;
    uint8_t buff[24];
    buff[0] = REG_6X8ENSTR;
...
/* 
 * Function ScI2cMxDisplay6x8DecNum
 * Desc     ZT.SC-I2CMx Display 6x8 number
 * Input    addr:ZT.SC-I2CMx Address
            page: location page
            column: location column
            num: number, 0~65535(0~0xFFFF)
 * Output   0 .. success
 *          1 .. length to long for buffer
 *          2 .. address send, NACK received
 *          3 .. data send, NACK received
 *          4 .. other twi error (lost bus arbitration, bus error, ..)
 */
int ZtLib::ScI2cMxDisplay6x8DecNum(uint8_t addr, uint8_t page, uint8_t column, uint16_t num,uint8_t disbit)
{
    uint8_t i=0;
    uint8_t j=0;
    uint8_t numbit=0;
    ...
    buff_num[0] = (num/10000)+'0'; //65535
    buff_num[1] = ((num/1000)%10)+'0';
    buff_num[2] = ((num/100)%10)+'0';
    buff_num[3] = ((num%100)/10)+'0';//1320
    buff_num[4] = (num%10)+'0';//12
    
    if (num>9999) numbit=5;
    else if(num>999) numbit=4;
    else if(num>99) numbit=3;
    else if(num>9) numbit=2;
    else numbit=1;
    ...
    return twi_writeTo(addr, buff, numbit+3, 1, 1);
}
/* 
...
/* 
 * Function ScI2cMxDisplay8x16DecNum
 * Desc     ZT.SC-I2CMx Display 8x16 number
 * Input    addr:ZT.SC-I2CMx Address
            page: location page
            column: location column
            num: number, 0~65535(0~0xFFFF)
 ...
 *          4 .. other twi error (lost bus arbitration, bus error, ..)
 */
int ZtLib::ScI2cMxDisplay8x16DecNum(uint8_t addr, uint8_t page, uint8_t column, uint16_t num,uint8_t disbit)
{
    uint8_t i=0;
    uint8_t j=0;
    uint8_t numbit=0;
...
/* 
 * Function ScI2cMxDisplayGb2132Str
 * Desc     ZT.SC-I2CMx Display GB2132 String
 * Input    addr:ZT.SC-I2CMx Address
            page: location page
            column: location column
            *str: 8X16 English String
 ...
 *          4 .. other twi error (lost bus arbitration, bus error, ..)
 */
int ZtLib::ScI2cMxDisplayGb2132Str(uint8_t addr, uint8_t page, uint8_t column, const char *str, uint8_t len)
{
    uint8_t i=0;
    uint8_t buff[32];
    buff[0] = REG_16X16CNSTR;
    buff[1] = page;
    buff[2] = column;
    i=0;
    while ((len>0) && (i<29))
